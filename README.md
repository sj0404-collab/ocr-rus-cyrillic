...
Для камеры используйте `LiveOCRSession` в Python или `android/LiveOcrController.kt` в APK. Сигнатура уменьшенного grayscale-crop сравнивается с предыдущим кадром: если crop не изменился, OCR не запускается и возвращается тот же текст. Для нового crop запускается один анализ, а тяжёлый verifier можно оставлять только для низкой confidence.
