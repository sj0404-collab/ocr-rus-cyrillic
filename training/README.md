...
Пять эпох — это проверка конвейера и smoke baseline, а не финальное качество. Для 90%+ нужны реальные размеченные кадры из целевого APK и отдельный test split по документам.
